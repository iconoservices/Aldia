...
import { StickyNote, Link as LinkIcon, CheckSquare, Minus, Table as TableIcon, MessageSquare, Image as ImageIcon, Target, LayoutGrid, Trash2, Library, MoreHorizontal, Copy, BarChartHorizontal } from 'lucide-react';
...
export const LienzoDashboard = () => {
    const defaultBoards = { 
        root: [
            {
                id: 'default-shelf',
                type: 'shelf',
                x: 220, // Centers a 560px wide shelf around visual x=500
                y: 311, // Centers a 378px high shelf around visual y=500
                content: 'Proyectos Principales',
                columns: 4, 
                rows: 2     
            }
        ] 
    ...
    const [boards, setBoards] = useState<Record<string, CanvasItem[]>>(() => {
        const saved = localStorage.getItem('lienzo_boards');
        if (saved) {
            try {
    ...
    useEffect(() => {
        localStorage.setItem('lienzo_boards', JSON.stringify(boards));
    }, [boards]);
                                                        ...
                                                        <tr key={rIndex}>
                                                            {Array.from({ length: item.columns || 4 }).map((_, cIndex) => {
                                                                ...
                                                                <td 
                                                                    ...
                                                                    style={{ border: '1px solid #e2e8f0', padding: 0, verticalAlign: 'top', width: item.columnWidths?.[cIndex] ? `${item.columnWidths[cIndex]}px` : (item.type === 'gantt' ? (cIndex === 0 ? '200px' : '100px') : 'auto'), position: 'relative', minWidth: item.type === 'gantt' ? (cIndex === 0 ? '200px' : '100px') : undefined }}
                                                                >
